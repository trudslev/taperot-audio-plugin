...
The case rule is `ValueCase::wordsOnly`: a value carrying no unit and no digit is a choice name rather than a reading, so MODEL and SWITCH get the control name's treatment while a numeric reading keeps its case — `s` and `S` are different units. TapeRot and Gatecrasher arrived at that independently; Elmer's source argues even a choice name should be authored in caps in `Parameters.h` instead, so the host's automation lane agrees. Both arguments sit beside `nf::ReadoutFormat::ValueCase`, and this panel keeps what it has until the designers rule. Guarded on the control's own drag state, because a `SliderAttachment` also fires when a Program is applied and on every host automation step — without the guard the display latches onto whichever parameter was written last and flickers for the length of a song. It also stands down entirely during naming; the glass belongs to the name field until it commits or cancels.
